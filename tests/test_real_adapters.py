...
from tw_stock_ai.adapters.base import AdapterFetchRequest, AdapterFetchResult
...
from tw_stock_ai.adapters.news import HybridTaiwanMarketNewsAdapter, MopsListedCompanyNewsAdapter
...
def test_hybrid_news_adapter_uses_premium_feed_only_inside_window(monkeypatch) -> None:
    settings = Settings(
        news_data_provider="hybrid_taiwan_market_news",
        finmind_api_token="token",
        finmind_free_news_always_on=True,
        premium_news_provider_enabled=True,
    )
    adapter = HybridTaiwanMarketNewsAdapter(settings)
    request = AdapterFetchRequest(symbols=["2330"])

    def make_result(adapter_name: str, source_name: str) -> AdapterFetchResult:
        return AdapterFetchResult(
            adapter_name=adapter_name,
            dataset="news",
            status="ready",
            fetched_at=datetime.now(timezone.utc),
            raw_items=[{"record_key": source_name}],
            cleaned_items=[
                {
                    "symbol": "2330",
    ...
    monkeypatch.setattr(adapter.mops_adapter, "fetch", lambda req: make_result("mops", "mops"))
    monkeypatch.setattr(adapter.finmind_adapter, "fetch", lambda req: make_result("finmind", "finmind"))
    monkeypatch.setattr("tw_stock_ai.adapters.news.premium_news_window_active", lambda settings: False)

    off_window = adapter.fetch(request)

    assert len(off_window.cleaned_items) == 2
    assert off_window.metadata["free_finmind_used"] is True
    assert off_window.metadata["premium_provider_used"] is False
    assert off_window.metadata["premium_window_active"] is False

    monkeypatch.setattr("tw_stock_ai.adapters.news.premium_news_window_active", lambda settings: True)

    on_window = adapter.fetch(request)

    assert len(on_window.cleaned_items) == 2
    assert on_window.metadata["free_finmind_used"] is True
    assert on_window.metadata["premium_provider_used"] is False
    assert on_window.metadata["premium_window_active"] is True
