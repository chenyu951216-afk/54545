...
class HybridTaiwanMarketNewsAdapter(NewsDataAdapter):
    ...
    def fetch(self, request: AdapterFetchRequest) -> AdapterFetchResult:
        fetched_at = datetime.now(timezone.utc)
        premium_window = premium_news_window_active(self.settings)
        results = [self.mops_adapter.fetch(request)]
        free_finmind_enabled = bool(
            request.symbols
            and self.settings.finmind_free_news_always_on
            and self.settings.finmind_api_token
        )
        if free_finmind_enabled:
        ...
        merged = _merge_results(
            ...
            metadata={
                "provider": "hybrid",
                "scope": "mops_plus_finmind_free" if free_finmind_enabled else "mops_primary",
                "free_finmind_used": free_finmind_enabled,
                "premium_window_active": premium_window,
                "premium_provider_used": False,
            },
        )
