...
- Hardened Discord webhook delivery by normalizing legacy `discordapp.com` webhook URLs to `discord.com` and sending explicit `User-Agent` / `Accept` headers to reduce Cloudflare `403 error code 1010` blocks.
