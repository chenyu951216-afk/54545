...
from tw_stock_ai.models import DailyReportRun, Holding, NewsItem, ScreeningCandidate, ScreeningRun
...
def latest_news(limit: int = 20, session: Session = Depends(get_session)) -> list[dict]:
    safe_limit = max(1, min(limit, 100))
    items = session.scalars(
        select(NewsItem).order_by(NewsItem.published_at.desc(), NewsItem.id.desc()).limit(safe_limit)
    ).all()
    return [
        {
            "id": item.id,
