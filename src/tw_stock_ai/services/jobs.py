...
def build_scheduler() -> BackgroundScheduler:
    ...
    logger.info(
        "scheduler_configured timezone=%s prewarm=%02d:%02d close_refresh=%s %02d:%02d push=%02d:%02d weekdays=%s",
        settings.scheduler_timezone,
        settings.prewarm_hour,
        settings.prewarm_minute,
        settings.close_refresh_enabled,
        settings.close_refresh_hour,
        settings.close_refresh_minute,
    ...
    if settings.close_refresh_enabled:
        scheduler.add_job(
            func=run_daily_close_refresh_job,
            trigger="cron",
            day_of_week=settings.screening_weekdays,
            hour=settings.close_refresh_hour,
            minute=settings.close_refresh_minute,
            id="daily-market-close-refresh",
            replace_existing=True,
            coalesce=True,
            misfire_grace_time=daily_misfire_grace_seconds,
        )
    if settings.news_polling_enabled:
        scheduler.add_job(
            func=run_intraday_news_poll_job,
            trigger="interval",
...
def run_intraday_news_poll_job() -> dict:
    with SessionLocal() as session:
        refresh_run = refresh_intraday_news(session, trigger_source="sched_news")
        if refresh_run is None:
            return {"status": "skipped", "reason": "no_symbols"}
        return refresh_run.model_dump()


def run_daily_close_refresh_job() -> dict:
    with SessionLocal() as session:
        ...
        return {
            ...
            "as_of_date": screening_run.as_of_date.isoformat(),
            "universe_size": screening_run.universe_size,
        }
...
def refresh_intraday_news(
    ...
    *,
    ...
    if latest_screening is not None:
        candidate_symbols = _collect_deep_refresh_symbols(
            session,
            latest_screening.id,
            settings.news_fetch_max_symbols_per_run,
        )
    holding_symbols = session.scalars(select(Holding.symbol).order_by(Holding.created_at.desc())).all()
    symbols = sorted(
        {
            symbol.strip()
            for symbol in [*candidate_symbols, *holding_symbols]
            if symbol and symbol.strip()
        }
    )[: settings.news_fetch_max_symbols_per_run]
    if not symbols:
        logger.info("intraday_news_poll_skipped reason=no_symbols trigger_source=%s", trigger_source)
        return None
    ...
    refresh_run = coordinator.refresh_all(
        ...
        requests={
            "news": AdapterFetchRequest(
                symbols=symbols,
                start_date=today - timedelta(days=settings.treasure_news_lookback_days),
                end_date=today,
                market_code="ALL",
                force_refresh=force_refresh,
                cache_ttl_seconds=0,
    ...
    logger.info(
        "intraday_news_polled trigger_source=%s status=%s symbols=%s",
        trigger_source,
        refresh_run.status,
        symbols,
    )
...
def run_startup_catchup(
    ...
    *,
    ...
    close_refresh_time = current.replace(
        hour=settings.close_refresh_hour,
        minute=settings.close_refresh_minute,
        second=0,
        microsecond=0,
    ...
    news_status = "skipped"
    close_refresh_status = "skipped"
    dispatch_status = "skipped"
    dispatch_report_id: int | None = latest_report.id if latest_report is not None else None

    if settings.close_refresh_enabled and current >= close_refresh_time:
        latest_screening_date = latest_screening.as_of_date if latest_screening is not None else None
        if latest_screening_date is None or latest_screening_date < today:
            refreshed_run = refresh_screening_state(
                session,
                trigger_source="startup_catchup_close_refresh",
                force_refresh=True,
            )
            close_refresh_status = refreshed_run.as_of_date.isoformat()
    ...
    summary = {
        ...
        "close_refresh_status": close_refresh_status,
