...
SETTING_DEFINITIONS: list[SettingDefinition] = [
    ...
    SettingDefinition("close_refresh_enabled", "bool", "Close Refresh", "scheduler", "收盤後自動重抓市場資料並重跑選股"),
    SettingDefinition("close_refresh_hour", "int", "收盤刷新小時", "scheduler", "每日收盤後自動刷新小時，Asia/Taipei"),
    SettingDefinition("close_refresh_minute", "int", "收盤刷新分鐘", "scheduler", "每日收盤後自動刷新分鐘，Asia/Taipei"),
...
def get_settings_for_ui(session: Session) -> dict[str, dict]:
    ...
    return {
        ...
        "schedule_preview": (
            ...
            f"close_refresh {'on' if effective.close_refresh_enabled else 'off'} "
            f"{int(effective.close_refresh_hour):02d}:{int(effective.close_refresh_minute):02d} / "
...
def save_settings(session: Session, values: dict[str, str]) -> list[str]:
    ...
    if "close_refresh_hour" in parsed_values and not 0 <= int(parsed_values["close_refresh_hour"]) <= 23:
        raise ValueError("close_refresh_hour must be between 0 and 23")
    if "close_refresh_minute" in parsed_values and not 0 <= int(parsed_values["close_refresh_minute"]) <= 59:
        raise ValueError("close_refresh_minute must be between 0 and 59")
