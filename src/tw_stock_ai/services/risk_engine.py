...
def build_risk_plan(
    ...
    recent_range = None
    if indicators.recent_high_20d is not None and indicators.recent_low_20d is not None:
        recent_range = max(indicators.recent_high_20d - indicators.recent_low_20d, 0.0)

    minimum_target_move = risk_per_share * settings.risk_min_reward_risk_ratio
    measured_target_move = max((recent_range or 0.0) * 0.6, atr * settings.risk_entry_buffer_atr_multiplier * 4)
    target_move_1 = max(minimum_target_move, measured_target_move)
    target_move_2 = max(
        risk_per_share * settings.risk_take_profit2_rr,
        (recent_range or 0.0),
        target_move_1 + atr,
    )

    take_profit_1 = round(reference_entry + target_move_1, 2)
    take_profit_2 = round(reference_entry + target_move_2, 2)
    rr_ratio = round(target_move_1 / risk_per_share, 2) if risk_per_share > 0 else None
    ...
    return RiskPlan(
        ...
        metrics={
            ...
            "recent_range": round(recent_range, 2) if recent_range is not None else None,
            "target_move_1": round(target_move_1, 2),
            "target_move_2": round(target_move_2, 2),
        },
    )
