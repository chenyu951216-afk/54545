...
class OptimizationApplyService:
    ...
    def _recenter(self, value: float, neutral: float, pull: float, low: float, high: float) -> float:
        adjusted = value + (neutral - value) * pull
        return self._clamp(adjusted, low, high)
    ...
    def _enforce_drift_guard(self, policy: Dict[str, Any]) -> Dict[str, Any]:
        drift = policy.get("drift_alert", {}) if isinstance(policy.get("drift_alert"), dict) else {}
        severity = str(drift.get("severity", "none") or "none")
        if severity not in {"medium", "high", "critical"}:
            policy["drift_guard_active"] = False
            return policy

        policy["drift_guard_active"] = True
        policy["position_management_profile"] = "defensive"
        policy["drift_guard_note"] = ",".join(str(x) for x in list(drift.get("reasons", []) or [])[:3])

        min_entry_shift = {"medium": 0.008, "high": 0.016, "critical": 0.024}[severity]
        max_size_bias = {"medium": 0.95, "high": 0.88, "critical": 0.82}[severity]
        max_leverage_bias = {"medium": 0.96, "high": 0.90, "critical": 0.84}[severity]
        max_scale_in = {"medium": 0.08, "high": 0.02, "critical": 0.0}[severity]
        max_breakout_tolerance = {"medium": 0.04, "high": 0.02, "critical": 0.0}[severity]

        policy["entry_confidence_shift"] = self._clamp(
            max(self._safe_float(policy.get("entry_confidence_shift", 0.0), 0.0), min_entry_shift),
            -0.08,
            0.08,
        )
        policy["size_multiplier_bias"] = self._clamp(
            min(self._safe_float(policy.get("size_multiplier_bias", 1.0), 1.0), max_size_bias),
            settings.adaptive_size_floor,
            settings.adaptive_size_ceiling,
        )
        policy["leverage_bias"] = self._clamp(
            min(self._safe_float(policy.get("leverage_bias", 1.0), 1.0), max_leverage_bias),
            0.7,
            1.4,
        )
        policy["scale_in_aggression"] = self._clamp(
            min(self._safe_float(policy.get("scale_in_aggression", 0.0), 0.0), max_scale_in),
            -0.2,
            0.35,
        )
        policy["entry_aggression"] = self._clamp(
            min(self._safe_float(policy.get("entry_aggression", 0.0), 0.0), 0.0),
            -0.2,
            0.2,
        )
        policy["breakout_tolerance"] = self._clamp(
            min(self._safe_float(policy.get("breakout_tolerance", 0.0), 0.0), max_breakout_tolerance),
            -0.2,
            0.2,
        )
        if severity in {"high", "critical"}:
            policy["protection_profile"] = "tight"
        if severity == "critical":
            policy["exit_style"] = "fast"
        return policy
    ...
    def apply(self, day: str, adjustments: Dict[str, Any], consensus: Dict[str, Any]) -> Dict[str, Any]:
        ...
        policy["last_lesson_update"] = day

        if mode == "lock":
            policy = self._enforce_drift_guard(policy)
        ...
        # Pull parameters gently back toward neutral so the policy does not drift forever.
        recenter_pull = 0.10 if mode == "active" else 0.22
        policy["entry_confidence_shift"] = self._recenter(float(policy.get("entry_confidence_shift", 0.0) or 0.0), 0.0, recenter_pull, -0.08, 0.08)
        policy["size_multiplier_bias"] = self._recenter(float(policy.get("size_multiplier_bias", 1.0) or 1.0), 1.0, recenter_pull, settings.adaptive_size_floor, settings.adaptive_size_ceiling)
        policy["leverage_bias"] = self._recenter(float(policy.get("leverage_bias", 1.0) or 1.0), 1.0, recenter_pull, 0.7, 1.4)
        policy["breakout_bias"] = self._recenter(float(policy.get("breakout_bias", 0.0) or 0.0), 0.0, recenter_pull, -0.2, 0.2)
        policy["trend_follow_bias"] = self._recenter(float(policy.get("trend_follow_bias", 0.0) or 0.0), 0.0, recenter_pull, -0.2, 0.2)
        policy["entry_aggression"] = self._recenter(float(policy.get("entry_aggression", 0.0) or 0.0), 0.0, recenter_pull, -0.2, 0.2)
        policy["breakout_tolerance"] = self._recenter(float(policy.get("breakout_tolerance", 0.0) or 0.0), 0.0, recenter_pull, -0.2, 0.2)
        policy["pullback_preference"] = self._recenter(float(policy.get("pullback_preference", 0.0) or 0.0), 0.0, recenter_pull, -0.2, 0.2)
        policy["scale_in_aggression"] = self._recenter(float(policy.get("scale_in_aggression", 0.0) or 0.0), 0.0, recenter_pull, -0.2, 0.35)
        policy["scale_out_aggression"] = self._recenter(float(policy.get("scale_out_aggression", 0.0) or 0.0), 0.0, recenter_pull, -0.2, 0.35)
        policy["reentry_after_partial"] = self._recenter(float(policy.get("reentry_after_partial", 0.0) or 0.0), 0.0, recenter_pull, -0.2, 0.25)
        policy["protection_refresh_bias"] = self._recenter(float(policy.get("protection_refresh_bias", 0.0) or 0.0), 0.0, recenter_pull, -0.2, 0.35)
