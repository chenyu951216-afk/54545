    ...
    def evaluate_positions(self, positions: List[Dict[str, Any]], feature_map: Dict[str, Dict[str, Any]], account_summary: Dict[str, Any], pos_mode: str) -> List[Dict[str, Any]]:
        ...
        for pos in positions:
            ...
            if exit_decision.get("action") == "exit":
                exit_row = self.exit.close_position(
                    {
                        **pos_ctx,
                        **feat,
                        "protection_state": protection.get("protection_profile", "balanced"),
                        "protection_decision": protection,
                        "sizing_decision": sizing,
                        "management_decision": management,
                        "exit_decision": exit_decision,
                    },
                    exit_decision.get("reason", "ai_exit"),
                )
            elif exit_decision.get("action") == "partial_exit":
                exit_row = self.exit.partial_close_position(
                    {
                        **pos_ctx,
                        **feat,
                        "protection_state": protection.get("protection_profile", "balanced"),
                        "management_action": "partial_exit",
                        "lifecycle_stage": "exit_trim",
                        "protection_decision": protection,
                        "sizing_decision": sizing,
                        "management_decision": management,
                        "exit_decision": exit_decision,
                    },
                    exit_decision.get("reason", "ai_partial_exit"),
                    settings.lifecycle_reduce_fraction,
                )
                ...
                if action == "scale_in":
                    ...
                    mgmt_row = self.exit.partial_close_position(
                        {
                            **pos_ctx,
                            **feat,
                            "protection_state": protection.get("protection_profile", "balanced"),
                            "management_action": action,
                            "lifecycle_stage": management.get("lifecycle_stage", action),
                            "protection_decision": protection,
                            "sizing_decision": sizing,
                            "management_decision": management,
                            "exit_decision": exit_decision,
                        },
                        management.get("reason", action),
                        fraction,
                    )
